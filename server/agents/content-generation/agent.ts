...
export class ContentGenerationServerAgent extends BaseServerAgent {
  ...
  protected generatePrompt(): string {
    // This method won't be used since we're using specialized content generation methods
    return '';
  }
  ...
  protected async callLLM(prompt: string): Promise<string> {
    // This is handled by specialized content generation methods
    return '';
  }
  ...
  public async execute(input: { data: any; messages?: any[]; brandTheme?: any }): Promise<{ content: string; metadata?: any; error?: string }> {
    try {
      ...
      const hasWebContent = (this.context.data.webSearchContent?.length ?? 0) > 0;
      const hasUserContent = (this.context.data.userProvidedContent?.trim().length ?? 0) > 0;
      const hasUrlContent = (this.context.data.urlsExtractedContent?.length ?? 0) > 0;
      ...
      console.error('Content generation agent execution error:', error);
      return {
        content: '',
    ...
    return [
      {
        ...
        description: 'Generate marketing email content from various sources',
        inputSchema: marketingEmailSchema,
        outputSchema: marketingEmailSchema
      }
    ];
  }
}
