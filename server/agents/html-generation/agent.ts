...
export class HtmlGenerationServerAgent extends BaseServerAgent {
  ...
  public async execute(input: { data: any; messages?: any[]; brandTheme?: any }): Promise<{ content: string; metadata?: any; error?: string }> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        const htmlContent = this.sanitizeAndValidateHTML(response);
        ...
        console.error('HTML generation agent execution error:', errorMsg);
        lastError = errorMsg;
    ...
    return [
      {
        ...
        description: 'Generate HTML email templates from content',
